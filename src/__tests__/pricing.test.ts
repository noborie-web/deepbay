...
describe('ホワイトリスト: 許可されていないフィールドを拒否', () => {
  ...
  it('ebay_descriptionはPhase2まで含まれない', () => {
    expect(PRODUCT_WRITE_WHITELIST.has('ebay_description')).toBe(false)
  })

  it('ebay_imagesはPhase2まで含まれない', () => {
    expect(PRODUCT_WRITE_WHITELIST.has('ebay_images')).toBe(false)
  })
